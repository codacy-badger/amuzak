...
<?php if (!defined('INSTALL')) {
    exit;
        ...
        <div class="well">
            <p>
                <?php echo T_('This page handles the installation of the Ampache database and the creation of the amuzak.cfg.php file. Before you continue please make sure that you have the following prerequisites:'); ?>
            </p>
            <ul>
                <li><?php echo T_('A MySQL server with a username and password that can create/modify databases'); ?></li>
                <li><?php echo sprintf(T_('Your webserver has read access to the files %s and %s'), $prefix . '/sql/ampache.sql', $prefix . '/config/amuzak.cfg.php.dist'); ?></li>
            </ul>
            <p>
                <?php echo sprintf(T_("Once you have ensured that the above requirements are met please fill out the information below. You will only be asked for the required config values. If you would like to make changes to your Ampache install at a later date simply edit %s"), $prefix . '/config/amuzak.cfg.php'); ?>
            </p>
        </div>
<table class="table" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <td><?php echo sprintf(T_('%s is readable'), 'amuzak.cfg.php.dist'); ?></td>
        <td><?php echo debug_result(is_readable($prefix . '/config/amuzak.cfg.php.dist')); ?></td>
    ...
    <tr>
        <td><?php echo T_('amuzak.cfg.php is writable'); ?></td>
