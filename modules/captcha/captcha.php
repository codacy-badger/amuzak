...
class easy_captcha
{
    ...
    public function solved($in = null/* parameter only used in subclasses */)
    {
        ...
        elseif ($this->sent) {
            ...
            $ok = $ok  // either letters or math formula submitted
                    || isset($this->image) && $this->image->solved($in) || isset($this->text) && $this->text->solved($in);
    ...
    public function form($add_text = "&rarr;&nbsp;")
    {
        ...
        #-- assemble
        $HTML = //'<script type="text/javascript" language="JavaScript">if (document.getElementById("captcha")) { document.getElementById("captcha").parentNode.removeChild(document.getElementById("captcha")); }</script>' .   // workaround for double instantiations
    ...
    {
        $filename = $this->data_file();
        if (file_exists($filename)) {
            $saved = (array) @unserialize(fread(fopen($filename, "r"), 1 << 20));
            foreach ($saved as $i => $v) {
                $this->{$i} = $v;
            }
        } else {
            $this->log("captcha file does not exist $filename");
        }
    }
    ...
    public function save()
    {
        ...
        if ($filename = $this->data_file()) {
            $this->saved = file_put_contents($filename, serialize($this), LOCK_EX);
        }
    }
    ...
    public function delete()
    {
        ...
        if (isset($this->prev)) {
            foreach ($this->prev as $id) {
                if (@unlink($this->data_file($id)) === false) {
                    throw new \RuntimeException('Error, unable to unlink ' . $this->data_file($id));
                }
            }
        }
        // clean object
    ...
    public function straighten_temp_dir()
    {
        ...
        // clean up old files
        if ((rand(0, 100) <= 5) && ($dirhandle = opendir($dir))) {
            $t_kill = time() - CAPTCHA_TIMEOUT * 1.2;
            while (false !== ($filename = readdir($dirhandle))) {
                if ($filename[0] !== ".") {
                    if (filemtime("$dir/$filename") < $t_kill) {
                        if (@unlink("$dir/$filename") === false) {
                            throw new \RuntimeException('Error, unable to unlink ' . $dir . '/' . $filename);
                        }
                    }
                }
            }
...
class easy_captcha_graphic_image_waved extends easy_captcha_graphic
{
    ...
    public function get_2x2_greyscale(&$i, $x, $y)
    {
        ...
        $cXY = (@imagecolorat($i, $x + $dx, $y + $dy) & 0xFF) + (@imagecolorat($i, $x + $dx, $y + $dy + 1) & 0xFF) + (@imagecolorat($i, $x + $dx + 1, $y + $dy) & 0xFF) + (@imagecolorat($i, $x + $dx + 1, $y + $dy + 1) & 0xFF);
        $cXY = (int) ($cXY / 4);
